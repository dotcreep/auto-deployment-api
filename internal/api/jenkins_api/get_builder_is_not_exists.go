...
func GetBuilderIsNotExists(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		Json.NewResponse(false, w, nil, "nama item sudah digunakan", http.StatusOK, true)
		return
	}
	_, err = connect.GetCredentialsIsNotExists(ctx, name)
	if err != nil {
		Json.NewResponse(false, w, nil, "nama kredensial sudah digunakan", http.StatusOK, true)
		return
	}
	Json.NewResponse(true, w, true, "item dapat dibuat", http.StatusOK, nil)
}
